...
from commons import get_outoftime_evts
...
def generate_oot_evts(odf_dir):
    ...
    status = exec_task(task)
    if status != 0:
        logger.info("ERROR: Task epchain failed. Check {}/tmp.log for more information".format(odf_dir))
        raise Exception
...
if __name__ == '__main__':
    ...
    for obsid in obsid_list:
        try:
            ...
            print("Running step 4: generate images")
            generate_images(odf_dir, bin_size, pi)

            if obsid in oot_list:
                print("Running step 5: generate out-of-time events list")
                generate_oot_evts(odf_dir)
                print("Running step 6: generate out-of-time events image")
                generate_oot_image(odf_dir)
